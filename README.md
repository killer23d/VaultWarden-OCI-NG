...
Install Docker and Dependencies:sudo apt update
sudo apt install -y docker.io docker-compose jq
sudo usermod -aG docker $USER
newgrp docker


(For OCI Vault) Install OCI CLI:curl -L -o /tmp/oci_install.sh https://raw.githubusercontent.com/oracle/oci-cli/master/scripts/install/install.sh
bash /tmp/oci_install.sh --accept-all-defaults
oci setup config  # Configure with OCI user credentials


Configure Storage:mkdir -p data/{mariadb,bwdata,backups,caddy_data,caddy_config,caddy_logs,backup_logs,fail2ban,redis} caddy ddclient fail2ban/filter.d backup
chmod -R 777 data


Configure Secrets:
Option 1: Local Storage:
Create/edit settings.env:nano settings.env
...
Option 2: OCI Vault:
Create settings.env locally, then store in OCI Vault:echo -n "$(cat settings.env | base64)" > secrets.b64
...
Fetch during deployment:oci vault secret get --secret-id <secret-ocid> --raw-output | jq -r '.data."secret-content".content' | base64 -d > settings.env
